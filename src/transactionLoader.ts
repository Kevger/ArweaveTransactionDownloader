...
const ARWEAVE_BASE_URL: URL = 'https://arweave.net';
 ...
 */
async function fetchOffsetInformation(transactionID: TransactionID) {
    const offsetUrl: URL = `${ARWEAVE_BASE_URL}/tx/${transactionID}/offset`
    try {
        const offsetResponse = (await axios.get<ArwaeveOffset>(offsetUrl)).data
 ...
 */
export async function fetchTransactionData(transactionID: TransactionID, concurrencyLimit: number): Promise<Buffer> {
    ...
    const getChunk = async (offset: BigInt, index: number): Promise<void> => {
        const response = await axios.get<ArwaeveChunk>(`${ARWEAVE_BASE_URL}/chunk/${offset}`);
    ...
    let fetches: Array<Promise<void>> = [];
 ...
 * @return Uint8Array of the transaction data
 */
export async function fetchAllData(transactionID: TransactionID): Promise<Buffer> {
